...
mod tests {
    ...
    fn test_set_lock_node() {
        ...
        assert_eq!(tree.name, tmpfile.as_path().file_name().unwrap());
    ...
    fn test_walk_tree() {
        ...
        let mut count = 0;
        tree.walk_bfs(false, &mut |_n| -> bool {
            count += 1;
            true
        })
        .unwrap();
